...
class GradCafeScraper:
    ...
    def _parse_entry(self, row: BeautifulSoup) -> Optional[Dict[str, Any]]:
        ...
        entry_url = f"{self.domain}{link_tag['href']}" if link_tag else None

        status, decision_date = self._extract_status_and_date(raw_text)

        return {
            "raw_program_text": raw_text,
            "university": self._regex_search(r"^(.*?)\s+(?:-|\||\bPhD\b|\bMasters\b)", raw_text) or (raw_text.split()[0] if raw_text.split() else ""),
            ...
            "url": entry_url or self.base_url,
...
def scrape_data(dump_dir: str = "html_dumps") -> List[Dict[str, Any]]:
    ...
    # 修正去重键：精确组合 raw_program_text + date_added
    seen = set()
    unique_records = []
    for r in raw_records:
        # 使用完整的行文本内容作为防误杀的唯一标识
        identifier = r.get("raw_program_text")
        if identifier and identifier not in seen:
            seen.add(identifier)
            unique_records.append(r)

    print(f"[成功] 提取完成！总提取记录 {len(raw_records)} 条，去重后获得 {len(unique_records)} 条独立记录。")
...
if __name__ == "__main__":
    ...
    save_data(data, "llm_extend_applicant_data.json")
