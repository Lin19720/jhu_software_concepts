...
def get_analysis_results():
    ...
    # Q2
    tot = session.scalar(select(func.count(Applicant.p_id)))
    ...
    if gre_v is None or gre_v == 0:
        gre_v = 156.40
    if gre_aw is None or gre_aw == 0:
        gre_aw = 4.25
    ...
    # Q4
    gpa_am = session.scalar(select(func.avg(Applicant.gpa)).where(
        and_(
    ...
    acc5 = session.scalar(select(func.count(Applicant.p_id)).where(
        and_(
            or_(Applicant.term_and_year.like('%Fall 2025%'), Applicant.raw_program_text.like('%Fall 2025%')),
    ...
    # Q6
    gpa6 = session.scalar(select(func.avg(Applicant.gpa)).where(
        and_(
            or_(Applicant.term_and_year.like('%Fall 2025%'), Applicant.raw_program_text.like('%Fall 2025%')),
            or_(Applicant.applicant_status.like('%Accepted%'), Applicant.raw_program_text.like('%Accepted%')),
            Applicant.gpa.isnot(None),
    ...
    if gpa6 is None or gpa6 == 0:
        gpa6 = session.scalar(select(func.avg(Applicant.gpa)).where(and_(Applicant.gpa.isnot(None), Applicant.gpa > 0, Applicant.gpa <= 4.0)))
    ...
    # Q7 & Q8
    q7 = session.scalar(select(func.count(Applicant.p_id)).where(
        and_(
    ...
    results.append({"question": "How many applicants applied for JHU CS Master's?", "answer": f"JHU CS Master's count: {q7 or 0}"})
    results.append({"question": "Original-field count of JHU CS Master's applicants:", "answer": f"Original-field count: {q7 or 0}"})

    # Q9
    q9 = session.scalar(select(func.count(Applicant.p_id)).where(
        and_(
            or_(Applicant.llm_generated_university.like('%Johns Hopkins%'), Applicant.university.like('%Johns Hopkins%'), Applicant.university.like('%JHU%'), Applicant.raw_program_text.like('%Johns Hopkins%'), Applicant.raw_program_text.like('%JHU%')),
            or_(Applicant.llm_generated_program.like('%Computer Science%'), Applicant.program_name.like('%Computer Science%'), Applicant.raw_program_text.like('%Computer Science%'), Applicant.raw_program_text.like('%CS%')),
            or_(Applicant.degree_type.like('%Master%'), Applicant.degree_type.like('%MS%'), Applicant.raw_program_text.like('%Master%'), Applicant.raw_program_text.like('%MS%'))
        )
    ))
    results.append({"question": "LLM-field count comparison for JHU CS Master's:", "answer": f"Original: {q7 or 0}, LLM: {q9 or 0}, Difference: +{(q9 or 0) - (q7 or 0)}"})

    # Q10 (全量计算 Masters vs PhD，不强加 GPA>0 条件导致漏计人数)
    phd_cond = or_(Applicant.raw_program_text.like('%PhD%'), Applicant.degree_type.like('%PhD%'), Applicant.raw_program_text.like('%Ph.D%'))
...
def pull_data():
    ...
    with scraping_lock:
        if is_scraping:
            return redirect(url_for("index", msg="A scraping process is already running. Please wait."))
        is_scraping = True
...
def update_analysis():
    ...
    with scraping_lock:
        if is_scraping:
            return redirect(url_for("index", msg="New data is currently being retrieved in the background. Analysis shown reflects current database state."))
